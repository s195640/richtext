...
// Height of the mock app bar below — the content-editor toolbar is told to
// offset its own sticky positioning by this, so it doesn't scroll under it.
...
export default function App() {
  ...
  return (
    <>
      <div className={styles.appBar}>Host App Nav Bar (mock — demonstrates toolbarOffset)</div>
      <div className={styles.page}>
        <header className={styles.header}>
          <div>
            <h1>Content Editor — Playground</h1>
            <p className={styles.sub}>
              One Tiptap document, mock uploads, no backend. Edit it, save it to <code>localStorage</code> as JSON,
              then check the read-only viewer renders the same thing.
            </p>
          </div>
        ...
        {savedAt && (
          <div className={styles.savedNote}>
            Saved at {savedAt} — {JSON.stringify(savedDoc).length} bytes of JSON. Active: {entryActive ? "on" : "off"}
            .
          </div>
        )}
